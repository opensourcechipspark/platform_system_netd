...
#define LOG_TAG "SoftapController_mt5931"
...
#undef AP_BSS_START_DELAY
#undef AP_BSS_STOP_DELAY
#undef AP_SET_CFG_DELAY
#undef AP_DRIVER_START_DELAY
...
SoftapController_mt5931::SoftapController_mt5931() {
...
int SoftapController_mt5931::setCommand(char *iface, const char *cmd, unsigned buflen) {
    int connectTries = 0;
...
int SoftapController_mt5931::startDriver(char *iface) {
    int ret;
...
int SoftapController_mt5931::stopDriver(char *iface) {
    int ret;
...
int SoftapController_mt5931::startSoftap() {
    pid_t pid = 1;
    int ret = 0;
...
int SoftapController_mt5931::stopSoftap() {
    int ret;
...
bool SoftapController_mt5931::isSoftapStarted() {
    ALOGV("isSoftapStarted: %s", (mPid != 0 ? "TRUE" : "FALSE"));
    return (mPid != 0 ? true : false);
}

int SoftapController_mt5931::setConfig(const char *cmd, const char *arg)
{
 ...
 *  argv[7] - Key
 */
int SoftapController_mt5931::setSoftap(int argc, char *argv[]) {
    ...
    if (argc > 5) {
        ret = setConfig("ch", argv[5]);
    } else {
        ret = setConfig("ch", "1");
    }
    if (argc > 6) {
        sprintf(str_arg, "\"%s\"", argv[6]);
        ret = setConfig("sec", str_arg);
    } else {
        ret = setConfig("sec", "\"open\"");
    }
    if (argc > 7) {
        sprintf(str_arg, "\"%s\"", argv[7]);
...
void SoftapController_mt5931::generatePsk(char *ssid, char *passphrase, char *psk_str) {
 ...
 *	argv[3] - AP or STA
 */
int SoftapController_mt5931::fwReloadSoftap(int argc, char *argv[])
{
    int ret, i = 0;
    char *iface;
    char *fwpath;
...
int SoftapController_mt5931::clientsSoftap(char **retbuf)
{
    int ret;
...
int SoftapController_mt5931::getChannelList(int buf_len, char *buf_list)
{
    struct iwreq wrq = {0};
    int i = 0, skfd = 0;
